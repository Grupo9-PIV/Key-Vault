...
from typing import Union # essa função declara que uma variável pode ter vários tipos de dados
...
class LicenseService:
    ...
    @staticmethod
    def validate_license_type(license_type: Union[str, LicenseType, None]) -> Union[str, None]:
        """
        ...
        try:
            return LicenseType(license_type_lower).value  # Retorna o valor do enum como string
        except ValueError:
    ...
    def create_license(db: Session, license_data: LicenseCreate):
        ...
        # Valida e mapeia o tipo de licença
        validated_data['license_type'] = LicenseService.validate_license_type(validated_data.get('license_type'))
        ...
        if software_name in LicenseService.SOFTWARE_LICENSE_RULES:
            ...
            expected_length = LicenseService.SOFTWARE_LICENSE_RULES[software_name]
            
            # Regex para verificar se a chave é alfanumérica e tem o tamanho esperado
            if not re.fullmatch(r'^[A-Za-z0-9]{' + str(expected_length) + r'}$', license_key):
                raise ValueError(
                    f"A chave de licença para {software_name.capitalize()} deve conter exatamente {expected_length} caracteres alfanuméricos."
                )
        
        new_license = License(**validated_data) #desempacota o dicionário em argumentos  
        db.add(new_license)
        db.commit()
    ...
    def update_license(db: Session, license_id: int, license_data: LicenseUpdate):
        ...
        if 'license_type' in validated_data:
            validated_data['license_type'] = LicenseService.validate_license_type(validated_data['license_type'])
