...
def create_license(db: Session, license_data: LicenseCreate):
    ...
    existing_license = (
        db.query(License)
        .filter(License.license_key == license_data.code)
        .first()
    )
    if existing_license:
        # Levanta exceção se o código já existe
        raise LicenseCodeAlreadyExistsException()
...
def update_license(db: Session, license_id: int, license_data: LicenseUpdate):
    ...
    if license_obj:
        for field, value in license_data.model_dump().items():
